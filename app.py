...
if page == "Home":
    ...
    if st.button('Predict Salary'):
        ...
        with col1:
            st.image("/content/minimum_img.png", use_column_width=False, width=70)
            st.markdown(f"### Predicted Minimum Annual Salary")
            st.write(f'**₹{min_salary_in_inr:,.2f} INR**')

        with col2:
            st.image("/content/maximum_image.png", use_column_width=False, width=70)  # Reduce image size by specifying width
